...
<%session.removeAttribute("refresh");%>
		...
		function choose(categoryName){
			...
			//alert(changeValue.value);
			document.getElementById("showCategory").value = categoryName;
  ...
  <body style="background-color: white;">
	...
	<div class="row clearfix">
		<div class="col-md-12 column">
			<%--<form action="${pageContext.request.contextPath }/group_findGroup?page=1" method="get">--%>
 				   <label for="chooseGroup">选择集群编号</label>
				   <input type="text" id="showCategory" width="10px" name="groupId">
				   		<%--<input type="hidden" id="theHide1">--%>
						<button name="chooseGroup" data-toggle="dropdown" class="btn btn-default dropdown-toggle" >
...
<!-- 				<li>1</li>	
			...
			<label for="animalId">动物编号</label>
				<input type="text" name="animalId" onblur="checkAnimalId()" id="animalId">
				<span id="span1"></span>
				<%--<input type="submit" value="开始查找">--%>
				<button type="button" onclick="showGroup()">查找</button>
				<button type="button" onclick="showGroup2()">查看全部集群</button>
				<%--<a href="${pageContext.request.contextPath }/group_findAllGroup"><button type="button">按集群查看</button></a>--%>
				<script type="text/javascript">
                    function showGroup() {
                        //alert("show");
                        ...
                        //alert(theForm);
                        //theForm.submit();
                        var groupId=$("#showCategory").val();
                        var animalId=$("#animalId").val();
                        var url = "${pageContext.request.contextPath }/group_findGroup";
                        var data = {"groupId":groupId , "animalId":animalId ,"page":1};
                        $.ajax({
                            type : "get",
                            async : false,  //同步请求
                            url : url,
                    ...
                    function showGroupByPage(thePage) {
                        ...
                        var animalId=$("#animalId").val();
                        var url = "${pageContext.request.contextPath }/group_findGroup";
                        var data = {"groupId":groupId , "animalId":animalId ,"page":thePage};
                        $.ajax({
                            type : "get",
                            async : false,  //同步请求
                            url : url,
                    ...
                    function showGroup2() {
                        ...
                        var url = "${pageContext.request.contextPath }/group_findAllGroup";
                        var data = {"page":1};
                        $.ajax({
                            type : "get",
                            async : false,  //同步请求
                    ...
                    function showGroupByPage2(thePage) {
                        //alert("show");
                        //var theForm = $("#showForm");
                        //alert(theForm);
                        //theForm.submit();
			...
			<table class="table table-bordered">
				...
				<s:iterator var="p" value="pageBean.list">
					<tr>
						...
						<td>
							<a href="${pageContext.request.contextPath }/feed_toModifyFeed?animalId=${p.animal.animalId}&groupId=${p.groupInformation.groupId}">更改</a>
							<br>
							<a href="${pageContext.request.contextPath }/feed_deleteOne?animalId=${p.animal.animalId}&groupId=${p.groupInformation.groupId}">删除</a>
						</td>
						</tr>
				</s:iterator>
				</tbody>
			...
			<table class="table table-bordered">
				...
				<s:iterator var="p" value="allGroup.list">
					<tr>
						...
						<td>
							<a href="${pageContext.request.contextPath }/group_findByGroupId?groupId=${p}" target="_blank">查看详细</a>
							<br>
